...
- [x] B95 Add a draggable left edge to the right chat panel, with keyboard arrow/Home/End resizing and saved per-account browser width. Hide resize handle in mobile tab layout. Verified pointer and keyboard resizing, manuscript reflow, production build, and Blade compilation.

- [x] B96 Add Laravel English/Turkish JSON catalogs and validation/auth/password/pagination translations. Add a header language switch, visitor session/cookie preference, account locale migration, and locale-aware account notifications. Localize Blade, JS UI, controller/service errors, country names, and dynamic labels without translating manuscript content or changing codex/API identifiers. JavaScript reads a filtered Laravel catalog; build/dev regenerate the frontend key list.
- [x] B97 Verify localization with persisted account and guest preferences, English/Turkish API validation, unchanged manuscript/type values, matching catalog placeholders, and public desktop/mobile browser checks. Full suite: 74 tests / 1078 assertions; assets built. Bulk rewrite was replaced by inspected, syntax-validated staging before application.

- [x] B98 Add APP_LOCALE=en to .env and .env.example; support en/tr as the configured default for visitors and accounts without a saved preference. Preserve account/session/cookie choices, safely fall back to English for unsupported configuration, and keep the default independent of per-request locale changes. Four focused localization tests pass.

- [x] B99 Fix timeout lockouts caused by PHP's 60-second execution limit: allow 260 seconds for the two-stage workflow, retain 120 seconds per OpenRouter call, and add bounded browser waits (120 seconds generally; 270 for two-stage chat). Replace expiring cache locks with OS-held book locks released on completion or process death. Each manual send gets a fresh request ID; inputs unlock before state refresh.
- [x] B100 Add English/Turkish progress dialog after seven seconds, manual dismissal and automatic closure on success/error; add animated spinners beside the Thinking button and footer status. Verified dialog timing/dismissal/cleanup in browser, timeout-then-send recovery, parallel exclusion, and lock recovery after an actual isolated PHP fatal timeout. Full suite: 77 tests / 1106 assertions; production assets built.

- [x] B101 Hide orphaned chat proposal cards when their original assistant reply is unavailable and the proposal is rejected. Keep pending/approved orphaned proposals and change buttons attached to existing replies visible. Production assets rebuilt.

- [x] B102 Add ALLOW_LANGUAGE_CHANGE=yes/no in .env and .env.example (missing defaults to no). When disabled hide the language switch, reject language-update requests, and force APP_LOCALE without deleting saved preferences. Verified guest/account enforcement and restoring saved language on re-enable; five localization tests pass.

- [x] B103 Track pending/completed/failed chat submissions and recognize interrupted pending requests once their process lock is released. Backfill known historical failure replies. Add localized edit icon to failed user messages: delete from chat, restore original prompt to the input, and focus for manual resend; confirm before replacing a different draft. Browser verified deletion/refill/focus without auto-send. Migration applied, assets rebuilt, and 79 tests / 1130 assertions pass.

- [x] B104 Capture all eight Turkish landing screenshots from book 3 through normal account login, matching the English viewports and dialog crops. Select -TR.png assets for Turkish and unsuffixed assets for English; localize book captions and missing history label. Verify both locales and image loading without changing manuscript, codex, chat, or revisions.

- [x] B105 Generate Net Kitap raster NK branding matching the existing paper logo; export original 1254px, 128px, 256px and 64px favicon. Save as separate net-kitap assets and leave environment paths unchanged.

- [x] B106 Set local .env branding to Net Kitap with net-kitap-logo-256.png and net-kitap-favicon.png; clear cached config and recapture all eight Turkish landing PNGs from book 3 with the new branding. Keep localhost URL and English screenshots unchanged.
