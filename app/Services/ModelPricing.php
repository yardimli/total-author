...
class ModelPricing
{
    public function ceilings(array $pricing): array
    {
        ...
        foreach (['prompt', 'completion'] as $key) {
            abort_unless(isset($pricing[$key]), 422, 'This model does not have bounded pricing.');
        }
        $overrides = $pricing['overrides'] ?? [];
        abort_unless(is_array($overrides), 422, 'This model does not have bounded pricing.');
        // Reserve the highest advertised rate, including conditional context/time tiers.
        foreach ([$pricing, ...$overrides] as $tier) {
            abort_unless(is_array($tier), 422, 'This model does not have bounded pricing.');
            foreach ($tier as $key => $value) {
                if (array_key_exists($key, $rates)) {
                    abort_unless(is_numeric($value) && is_finite((float) $value) && $value >= 0, 422, 'This model does not have bounded pricing.');
                    ...
                    abort_unless(is_numeric($value) && (float) $value === 0.0, 422, 'This model has an unsupported pricing fee. Choose another model.');
                }
            }
        }
